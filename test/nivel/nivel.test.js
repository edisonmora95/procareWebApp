...
describe('Niveles', () => {
    ...
    describe('Crear nivel', () => {
        ...
        it('CP1. Creación con datos correctos. Creación de nivel ', (done) => {
            chai.request(server)
                .post('/api/nivel/')
                .send(nivel)
                .end((err, res) => {
                    ...
                    assert.equal(res.body.mensaje, 'nivel creado exitosamente', 'Mensaje incorrecto');
                    done();
                });
        });
        it('CP2. Id de etapa ingresada es incorrecto', (done) => {
            nivel.nombre = 'Nivel 1';
            chai.request(server)
                .post('/api/nivel/')
                .send(nivel)
                .end((err, res) => {
        ...
        it('CP6. Número de reuniones mayor al posible', (done) => {
            ...
            chai.request(server)
                ...
                .end((err, res) => {
                    ...
                    res.should.have.status(400);
                    assert.equal(res.body.estado, false, 'Estado incorrecto');
                    assert.equal(res.body.mensaje, 'No se pudo crear el nivel', 'Mensaje incorrecto');
                    assert.equal(res.body.mensajeError, 'en programa no puede ser numeros enteros.', 'Mensaje de error incorrecto');
                    done();
                });
        });
        /*
